...
def conv_block(input, nb_filter, dropout_rate=None, weight_decay=1E-4):
    ...
    x = Convolution2D(nb_filter, (3, 3), kernel_initializer="he_uniform", padding="same", use_bias=False,data_format='channels_first',
...
def transition_block(input, nb_filter, dropout_rate=None, weight_decay=1E-4):
    ...
    x = Convolution2D(nb_filter, (1, 1), kernel_initializer="he_uniform", padding="same", use_bias=False,data_format='channels_first',
                      kernel_regularizer=l2(weight_decay))(input)
    if dropout_rate is not None:
        x = Dropout(dropout_rate)(x)
    x = AveragePooling2D((2, 2),data_format='channels_first', strides=(2, 2))(x)
...
def createDenseNet(nb_classes, img_dim, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=16, dropout_rate=None,
    ...
    # Initial convolution
    x = Convolution2D(nb_filter, (1, 1), kernel_initializer="he_uniform", padding="same", name="initial_conv2D", use_bias=False,data_format='channels_first',
